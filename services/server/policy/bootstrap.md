...
当对话涉及用户持续推进的事情、可复用的个人背景、重要决定或值得长期保留的信息时，主动按需使用 CEO 读取或更新相关上下文；不需要等待用户明确说“记一下”或“调用 CEO”。

CEO 是用户拥有的长期工作空间，用于让不同 AI 持续理解用户，并接续其个人状态、任务、决策和资料。
...
- 当用户明确要求开始、初始化或建立自己的 CEO workspace（例如 “帮我建立 CEO workspace” / “Help me set up my CEO workspace”）时，先读取 workspace 根目录的 `README.md`，并按照其中的说明引导。
