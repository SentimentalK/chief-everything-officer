...
import type { JobService, JobAuthScope, JobErrorCode, AssignmentResult, ReportResult } from "./service.js";
...
export function createJobAssignmentRouter(service: JobService | null): Router {
  ...
  router.post("/:job_id/report", async (req: Request, res: Response) => {
    ...
    const finishOk = (ok: Extract<ReportResult, { ok: true }>): void => {
      logLine({
        event: "report",
        job_id: jobId,
        worker_id: ids.worker_id,
      ...
      res.status(200).json({
        ...
        received_at: ok.received_at,
        replayed: ok.replayed,
      });
    };

    const finishErr = (e: Errorish): void => {
      logLine({
        event: "report",
        job_id: jobId,
        worker_id: ids.worker_id,
    ...
    if (!jobId) {
      finishErr({ code: "INVALID_INPUT", message: "job_id must be a job-<uuid>" });
      return;
    }
    if (!scope) {
      finishErr({ code: "INVALID_INPUT", message: "Missing identity context." });
      return;
    }
    if (!service) {
      finishErr({ code: "BRIDGE_DISABLED", message: "Job submission is disabled on this deployment." });
      return;
    }

    try {
      const result = await service.report(scope, jobId, req.body);
      if (result.ok) {
        finishOk(result);
        return;
      }
