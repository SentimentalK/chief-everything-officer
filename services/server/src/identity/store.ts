...
export interface AdoptExistingWorkspaceRepositoryInput {
  workspaceId: string;
...
export type AdoptExistingWorkspaceAction = "CREATE_ADOPTION" | "ALREADY_ADOPTED" | "CONFLICT";

export interface AdoptExistingWorkspacePreview {
  action: AdoptExistingWorkspaceAction;
  reason: string | null;
  workspace: {
    id: string;
  ...
  binding: GitHubRepositoryBindingRecord | null;
  bootstrap: WorkspaceBootstrapRecord | null;
}

export interface AdoptExistingWorkspaceResult {
  outcome: "CREATED" | "ALREADY_ADOPTED";
  workspace: {
    id: string;
    owner_user_id: string;
    remote_url: string;
...
export class IdentityStore {
  ...
  previewAdoptExistingWorkspaceRepository(
    input: AdoptExistingWorkspaceRepositoryInput,
  ): AdoptExistingWorkspacePreview {
    this.assertAdoptExistingWorkspaceInput(input);
    return this.withDb((db) => evaluateAdoptExistingWorkspace(db, input).preview);
  }

  adoptExistingWorkspaceRepository(
    input: AdoptExistingWorkspaceRepositoryInput,
    ...
    return this.withDb((db) => {
      db.exec("BEGIN IMMEDIATE;");
      try {
        const evaluated = evaluateAdoptExistingWorkspace(db, input);
        if (evaluated.kind === "invariant") {
          throw new IdentityStructureError(evaluated.preview.reason ?? "Existing workspace adoption invariant failure.");
        }
        if (evaluated.kind === "conflict") {
          throw new IdentityConflictError(evaluated.preview.reason ?? "Existing workspace adoption conflict.");
        }
        if (evaluated.kind === "already") {
          db.exec("ROLLBACK;");
          return {
            outcome: "ALREADY_ADOPTED" as const,
            workspace: evaluated.workspace!,
            membership: evaluated.membership!,
        ...
        `).run(input.workspaceId, nowMs, nowMs);
        ...
        return {
          ...
          binding: {
            ...
            github_repository_id: input.githubRepositoryId,
            github_installation_row_id: input.githubInstallationRowId,
            owner_account_id: input.ownerAccountId,
            owner_login: input.ownerLogin.trim(),
            repository_name: input.repositoryName.trim(),
            full_name: input.fullName.trim(),
        ...
        if (error instanceof IdentityError) throw error;
        throw new IdentityDbUnavailable(
          `Failed to adopt existing workspace repository: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    });
  }

  private assertAdoptExistingWorkspaceInput(input: AdoptExistingWorkspaceRepositoryInput): void {
    const numericRegex = /^[1-9][0-9]*$/;
    if (typeof input.workspaceId !== "string" || input.workspaceId.trim().length === 0) {
      throw new IdentityStructureError("workspaceId must be a non-empty string.");
    }
    if (typeof input.expectedOwnerUserId !== "string" || input.expectedOwnerUserId.trim().length === 0) {
      throw new IdentityStructureError("expectedOwnerUserId must be a non-empty string.");
    }
    if (typeof input.githubInstallationRowId !== "string" || input.githubInstallationRowId.trim().length === 0) {
      throw new IdentityStructureError("githubInstallationRowId must be a non-empty string.");
    }
    if (typeof input.githubRepositoryId !== "string" || !numericRegex.test(input.githubRepositoryId)) {
      throw new IdentityStructureError(
        `githubRepositoryId must be a positive decimal string, got '${input.githubRepositoryId}'.`,
    ...
    if (typeof input.expectedExistingRemoteUrl !== "string") {
      throw new IdentityStructureError("expectedExistingRemoteUrl must be a string.");
    }
  }
...
interface EvaluatedAdoption {
  kind: "create" | "already" | "conflict" | "invariant";
  preview: AdoptExistingWorkspacePreview;
  workspace: AdoptExistingWorkspaceResult["workspace"] | null;
  membership: AdoptExistingWorkspaceResult["membership"] | null;
  binding: GitHubRepositoryBindingRecord | null;
  bootstrap: WorkspaceBootstrapRecord | null;
}

function selectWorkspaceRow(db: DatabaseSync, workspaceId: string) {
  return db.prepare(
    "SELECT id, owner_user_id, remote_url, branch, created_at FROM workspaces WHERE id = ? LIMIT 1;",
  ).get(workspaceId) as
    | { id: string; owner_user_id: string; remote_url: string; branch: string; created_at: number }
...
function selectOwnerMembershipRow(db: DatabaseSync, workspaceId: string) {
  return db.prepare(
    "SELECT id, workspace_id, user_id, role, created_at FROM workspace_memberships WHERE workspace_id = ? AND role = 'owner' LIMIT 1;",
  ).get(workspaceId) as
    | { id: string; workspace_id: string; user_id: string; role: string; created_at: number }
...
function selectBindingByWorkspace(db: DatabaseSync, workspaceId: string): GitHubRepositoryBindingRecord | null {
  const row = db.prepare(`
    SELECT id, workspace_id, github_repository_id, github_installation_row_id, owner_account_id, owner_login, repository_name, full_name, branch, created_at_ms, updated_at_ms
    FROM github_repository_bindings
    WHERE workspace_id = ?
...
function selectBindingByRepoId(db: DatabaseSync, githubRepositoryId: string): GitHubRepositoryBindingRecord | null {
  const row = db.prepare(`
    SELECT id, workspace_id, github_repository_id, github_installation_row_id, owner_account_id, owner_login, repository_name, full_name, branch, created_at_ms, updated_at_ms
    FROM github_repository_bindings
    WHERE github_repository_id = ?
...
function selectBootstrapByWorkspace(db: DatabaseSync, workspaceId: string): WorkspaceBootstrapRecord | null {
  const row = db.prepare(`
    SELECT workspace_id, bootstrap_version, state, attempt_count,
           last_attempt_id, last_base_commit_sha, ready_commit_sha,
           last_error_kind, last_error_code, last_error_message,
...
function adoptionPreview(
  action: AdoptExistingWorkspaceAction,
  reason: string | null,
  workspace: EvaluatedAdoption["workspace"],
  membership: EvaluatedAdoption["membership"],
  binding: GitHubRepositoryBindingRecord | null,
  bootstrap: WorkspaceBootstrapRecord | null,
): AdoptExistingWorkspacePreview {
  return { action, reason, workspace, membership, binding, bootstrap };
}

function evaluateAdoptExistingWorkspace(
  db: DatabaseSync,
  input: AdoptExistingWorkspaceRepositoryInput,
): EvaluatedAdoption {
  const workspace = selectWorkspaceRow(db, input.workspaceId) ?? null;
  if (!workspace) {
    const reason = `Workspace '${input.workspaceId}' does not exist.`;
    return {
      kind: "invariant",
      preview: adoptionPreview("CONFLICT", reason, null, null, null, null),
      workspace: null,
      membership: null,
      binding: null,
      bootstrap: null,
  ...
  if (workspace.id !== input.workspaceId) {
    const reason = `Workspace ID mismatch: expected '${input.workspaceId}', found '${workspace.id}'.`;
    return {
      kind: "invariant",
      preview: adoptionPreview("CONFLICT", reason, workspace, null, null, null),
      workspace,
      membership: null,
      binding: null,
      bootstrap: null,
  ...
  const membership = selectOwnerMembershipRow(db, input.workspaceId) ?? null;
  if (!membership) {
    const reason = `Workspace '${input.workspaceId}' has no owner membership.`;
    return {
      kind: "invariant",
      preview: adoptionPreview("CONFLICT", reason, workspace, null, null, null),
      workspace,
  ...
  if (membership.user_id !== input.expectedOwnerUserId) {
    const reason = `Owner membership user '${membership.user_id}' does not match expected owner '${input.expectedOwnerUserId}'.`;
    return {
      kind: "invariant",
      preview: adoptionPreview("CONFLICT", reason, workspace, membership, null, null),
      workspace,
      membership,
      binding: null,
      bootstrap: null,
  ...
  if (workspace.owner_user_id !== membership.user_id) {
    const reason = `Workspace '${workspace.id}' shadow owner '${workspace.owner_user_id}' does not match owner membership user '${membership.user_id}'.`;
    return {
      kind: "invariant",
      preview: adoptionPreview("CONFLICT", reason, workspace, membership, null, null),
      workspace,
  ...
  const userRow = db.prepare(
    "SELECT id, disabled_at FROM users WHERE id = ? LIMIT 1;",
  ).get(input.expectedOwnerUserId) as { id: string; disabled_at: number | null } | undefined;
  if (!userRow) {
    const reason = `Expected owner user '${input.expectedOwnerUserId}' does not exist.`;
    return {
      kind: "invariant",
      preview: adoptionPreview("CONFLICT", reason, workspace, membership, null, null),
      workspace,
  ...
  if (userRow.disabled_at != null) {
    const reason = `Expected owner user '${input.expectedOwnerUserId}' is disabled.`;
    return {
      kind: "invariant",
      preview: adoptionPreview("CONFLICT", reason, workspace, membership, null, null),
      workspace,
  ...
  const instRow = db.prepare(
    "SELECT id FROM github_installations WHERE id = ? LIMIT 1;",
  ).get(input.githubInstallationRowId) as { id: string } | undefined;
  if (!instRow) {
    const reason = `GitHub installation row '${input.githubInstallationRowId}' does not exist.`;
    return {
      kind: "invariant",
      preview: adoptionPreview("CONFLICT", reason, workspace, membership, null, null),
      workspace,
  ...
  const userLink = db.prepare(
    "SELECT id FROM github_installation_users WHERE github_installation_row_id = ? AND user_id = ? LIMIT 1;",
  ).get(input.githubInstallationRowId, input.expectedOwnerUserId) as { id: string } | undefined;
  if (!userLink) {
    const reason = `User '${input.expectedOwnerUserId}' has no verified association with installation row '${input.githubInstallationRowId}'.`;
    return {
      kind: "invariant",
      preview: adoptionPreview("CONFLICT", reason, workspace, membership, null, null),
      workspace,
  ...
  if (workspace.branch !== input.branch.trim()) {
    const reason = `Workspace branch '${workspace.branch}' does not match requested branch '${input.branch.trim()}'.`;
    return {
      kind: "conflict",
      preview: adoptionPreview("CONFLICT", reason, workspace, membership, null, null),
      workspace,
  ...
  if (workspace.remote_url !== input.expectedExistingRemoteUrl) {
    const reason = `Workspace remote_url does not match expectedExistingRemoteUrl exactly.`;
    return {
      kind: "conflict",
      preview: adoptionPreview("CONFLICT", reason, workspace, membership, null, null),
      workspace,
  ...
  const bindingByWorkspace = selectBindingByWorkspace(db, input.workspaceId);
  const bootstrap = selectBootstrapByWorkspace(db, input.workspaceId);
  const bindingByRepo = selectBindingByRepoId(db, input.githubRepositoryId);

  if (bindingByWorkspace && !bootstrap) {
    const reason = `Invariant failure: workspace '${input.workspaceId}' has a repository binding but no bootstrap row.`;
    return {
      kind: "invariant",
      preview: adoptionPreview("CONFLICT", reason, workspace, membership, bindingByWorkspace, null),
      workspace,
      membership,
      binding: bindingByWorkspace,
      bootstrap: null,
    };
  }
  if (bootstrap && !bindingByWorkspace) {
    const reason = `Invariant failure: workspace '${input.workspaceId}' has a bootstrap row but no repository binding.`;
    return {
      kind: "invariant",
      preview: adoptionPreview("CONFLICT", reason, workspace, membership, null, bootstrap),
      workspace,
      membership,
      binding: null,
      bootstrap,
  ...
  if (bindingByWorkspace && bootstrap) {
    const intended =
      bindingByWorkspace.workspace_id === input.workspaceId &&
      bindingByWorkspace.github_repository_id === input.githubRepositoryId &&
      bindingByWorkspace.github_installation_row_id === input.githubInstallationRowId &&
      bindingByWorkspace.owner_account_id === input.ownerAccountId &&
      bindingByWorkspace.branch === input.branch.trim();
    if (intended) {
      return {
        kind: "already",
        preview: adoptionPreview("ALREADY_ADOPTED", null, workspace, membership, bindingByWorkspace, bootstrap),
        workspace,
        membership,
        binding: bindingByWorkspace,
        bootstrap,
    ...
    const reason = `Workspace '${input.workspaceId}' already has a conflicting repository binding.`;
    return {
      kind: "conflict",
      preview: adoptionPreview("CONFLICT", reason, workspace, membership, bindingByWorkspace, bootstrap),
      workspace,
      membership,
      binding: bindingByWorkspace,
      bootstrap,
  ...
  if (bindingByRepo && bindingByRepo.workspace_id !== input.workspaceId) {
    const reason = `GitHub repository '${input.githubRepositoryId}' is already bound to workspace '${bindingByRepo.workspace_id}'.`;
    return {
      kind: "conflict",
      preview: adoptionPreview("CONFLICT", reason, workspace, membership, bindingByRepo, bootstrap),
      workspace,
      membership,
      binding: bindingByRepo,
      bootstrap,
    };
  }
  ...
  return {
    kind: "create",
    preview: adoptionPreview("CREATE_ADOPTION", null, workspace, membership, null, null),
    workspace,
    membership,
    binding: null,
    bootstrap: null,
