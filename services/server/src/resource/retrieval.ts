...
import { readBlobUtf8 } from "../git.js";
import type {
  ResourceCard,
  ResourceGetInput,
  ResourceGetView,
...
import {
  enumerateResourcesAtSnapshot,
  resolveResourceLocationAtSnapshot,
} from "./locator.js";
...
export class ResourceRetrievalService {
  ...
  async search(input: ResourceSearchInput = {}): Promise<Record<string, unknown>> {
    ...
    const snapshot = await this.workspace.captureReadSnapshot();
    const all = await enumerateResourcesAtSnapshot(this.workspace.config, this.repoDir, snapshot);

    const entries: Array<{ card: ResourceCard; capturedMs: number }> = [];

    for (const item of all) {
      const { meta, doc, location, artifactSet, artifactEntries } = item;
      ...
      if (artifactSet.has("interactions.md")) {
        const entry = artifactEntries.get("interactions.md");
        if (entry) {
          interactionsText = await readBlobUtf8(
            this.workspace.config,
            this.repoDir,
            entry.oid,
            path.posix.join(location.relative_path, "interactions.md"),
          ).catch(() => null);
        }
      }

      let summaryText: string | null = null;
      const summaryPath = path.posix.join(location.relative_path, "summary.md");
      if (artifactSet.has("summary.md")) {
        const entry = artifactEntries.get("summary.md");
        if (entry) {
          summaryText = await readBlobUtf8(this.workspace.config, this.repoDir, entry.oid, summaryPath);
        }
      }

      const stage = deriveResourceStage(artifactSet, interactionsText, summaryText, summaryPath);
      const sourceAssetAvailable = artifactSet.has("source");
  ...
  async get(input: ResourceGetInput): Promise<Record<string, unknown>> {
    ...
    const { meta, doc, location, artifactSet, artifactEntries } = located;
    const { heading, capture_note, capture_history } = doc;
    ...
    const entry = artifactEntries.get(fileName);
    if (!entry) {
      return {
        ...commonHeader,
        view: requestedView,
    ...
    let text = await readBlobUtf8(
      this.workspace.config,
      this.repoDir,
      entry.oid,
      path.posix.join(location.relative_path, fileName),
    );
    ...
    if (requestedView === "summary") {
      const parsed = parseSummaryDocument(text, path.posix.join(location.relative_path, fileName));
      parsedSummaryMeta = {
