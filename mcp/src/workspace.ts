...
export class LifeOSWorkspace {
  ...
  async workspaceStatus(): Promise<Record<string, unknown>> {
    return await this.withLock(async () => {
      await this.recoverPending();
      if (this.state === "BLOCKED") throw new LifeOSError("WORKSPACE_DIVERGED", "Pending commit diverged from origin/main.");
      await this.syncCleanWorkspace();
  ...
  async listFiles(prefix = "", limit = 200): Promise<Record<string, unknown>> {
    return await this.withReadyWorkspace(async (base) => {
      if (prefix && (prefix.includes("..") || prefix.includes("\\") || path.posix.isAbsolute(prefix))) {
        throw new LifeOSError("INVALID_PATH", "Prefix must be a safe repository-relative path.", { prefix });
      }
  ...
  async readFiles(paths: string[]): Promise<Record<string, unknown>> {
    if (paths.length === 0 || paths.length > LIMITS.maxFilesPerRead) {
      throw new LifeOSError("VALIDATION_FAILED", `Read between 1 and ${LIMITS.maxFilesPerRead} files.`);
    }
    return await this.withReadyWorkspace(async (base) => {
      let total = 0;
      const files = [];
      for (const candidate of paths) {
        ...
        if (!info?.isFile()) throw new LifeOSError("INVALID_PATH", "Requested path is not a regular file.", { path: filePath });
        if (info.size > LIMITS.maxFileWriteBytes) throw new LifeOSError("VALIDATION_FAILED", `File size (${Math.round(info.size / 1024)} KiB) exceeds max single-file limit (${Math.round(LIMITS.maxFileWriteBytes / (1024 * 1024))} MiB).`, { path: filePath });
        total += info.size;
        if (total > LIMITS.maxReadResponseBytes) throw new LifeOSError("VALIDATION_FAILED", `Total response size exceeds response budget of ${Math.round(LIMITS.maxReadResponseBytes / (1024 * 1024))} MiB.`);
  ...
  async searchText(query: string, prefixes: string[], limit: number): Promise<Record<string, unknown>> {
    if (!query || Buffer.byteLength(query, "utf8") > 512) {
      throw new LifeOSError("VALIDATION_FAILED", "Search query must contain 1 to 512 UTF-8 bytes.");
    }
    return await this.withReadyWorkspace(async (base) => {
      ...
      for (const prefix of safePrefixes) {
        if (prefix.includes("..") || prefix.includes("\\") || path.posix.isAbsolute(prefix)) {
          throw new LifeOSError("INVALID_PATH", "Search prefix is invalid.", { prefix });
        }
      }
  ...
  async applyChangeSet(input: {
    ...
    if (!/^[0-9a-f-]{36}$/i.test(requestId)) throw new LifeOSError("VALIDATION_FAILED", "request_id must be a UUID.");
    if (!input.summary.trim() || input.summary.length > 120 || /[\r\n]/.test(input.summary)) {
      throw new LifeOSError("VALIDATION_FAILED", "Summary must be a single line of 1 to 120 characters.");
    }
    if (input.operations.length === 0 || input.operations.length > LIMITS.maxOperationsPerTransaction) {
      throw new LifeOSError("VALIDATION_FAILED", `Apply between 1 and ${LIMITS.maxOperationsPerTransaction} operations.`);
    }
    ...
    return await this.withLock(async () => {
      ...
      await this.recoverPending();
      if (await this.readPending()) throw new LifeOSError("PUSH_PENDING", "A previous commit is awaiting push verification.");
      await this.syncCleanWorkspace();
      const remote = await resolveRef(this.config, this.config.repoDir, `origin/${this.config.branch}`);
      if (remote !== input.base_commit) {
        throw new LifeOSError("STALE_REVISION", "origin/main changed since the files were read.", {
      ...
      try {
        ...
        await runGit(this.config, worktree, ["commit", "-m", `LifeOS: ${input.summary.trim()}`]);
        committed = true;
        ...
        if (latest !== input.base_commit) {
          this.state = "BLOCKED";
          throw new LifeOSError("STALE_REVISION", "origin/main moved during the transaction; the local commit was not pushed.", {
            remote_head: latest,
        ...
        try {
          ...
          throw new LifeOSError("PUSH_PENDING", "Commit created locally, but push could not be verified.", { commit });
        }
  ...
  private async withReadyWorkspace<T>(operation: (base: string) => Promise<T>): Promise<T> {
    return await this.withLock(async () => {
      await this.recoverPending();
      if (this.state === "BLOCKED") throw new LifeOSError("WORKSPACE_DIVERGED", "Workspace requires operator repair.");
      await this.syncCleanWorkspace();
  ...
  private async ensureRepository(): Promise<void> {
    ...
    if (!exists) {
      ...
      if (entries.length > 0) throw new LifeOSError("NOT_READY", "Repository directory is non-empty but is not a Git checkout.");
    ...
    if (origin !== this.config.remoteUrl) {
      throw new LifeOSError("NOT_READY", "Git origin does not match the configured LifeOS repository.");
    }
  }
  ...
  private async syncCleanWorkspace(): Promise<void> {
    ...
    if (dirty) throw new LifeOSError("WORKSPACE_DIRTY", "Main working copy contains uncommitted changes.");
    await runGit(this.config, this.config.repoDir, ["fetch", "origin", this.config.branch]);
    const local = await resolveRef(this.config, this.config.repoDir, "HEAD");
    const remote = await resolveRef(this.config, this.config.repoDir, `origin/${this.config.branch}`);
    ...
    try {
      ...
      this.state = "BLOCKED";
      throw new LifeOSError("WORKSPACE_DIVERGED", "Local and remote LifeOS history diverged.", { local, remote });
    }
  }
  ...
  private validateOperations(operations: ChangeOperation[]): void {
    ...
    for (const operation of operations) {
      ...
      if (operation.op === "append" && source !== "JOURNAL.md") {
        throw new LifeOSError("INVALID_OPERATION", "Append is only allowed for JOURNAL.md.");
      }
      if ("content" in operation) total += assertContentSize(operation.content);
      for (const filePath of paths) {
        if (touched.has(filePath)) throw new LifeOSError("INVALID_OPERATION", "A path may be changed only once per transaction.", { path: filePath });
        touched.add(filePath);
      }
    }
    if (total > LIMITS.maxTotalWriteBytes) throw new LifeOSError("VALIDATION_FAILED", `Transaction content exceeds ${Math.round(LIMITS.maxTotalWriteBytes / (1024 * 1024))} MiB.`);
  }
  ...
  private async applyOperations(worktree: string, base: string, operations: ChangeOperation[]): Promise<void> {
    for (const operation of operations) {
      ...
      if (operation.op === "create") {
        ...
        if (exists) throw new LifeOSError("INVALID_OPERATION", "Create target already exists.", { path: filePath });
        ...
        if (exists) throw new LifeOSError("INVALID_OPERATION", "Archive target already exists.", { target });
  ...
  private async validateDiff(worktree: string): Promise<string[]> {
    ...
    if (changed.length === 0) throw new LifeOSError("VALIDATION_FAILED", "Change set produces no file changes.");
  ...
  private async recoverPending(): Promise<void> {
    ...
    try {
      ...
      this.state = "PUSH_PENDING";
      throw new LifeOSError("PUSH_PENDING", "Cannot reach GitHub to recover the pending commit.", { commit: pending.commit });
    }
    const remote = await resolveRef(this.config, this.config.repoDir, `origin/${this.config.branch}`);
    ...
    if (remote === pending.base_commit) {
      try {
        ...
        this.state = "PUSH_PENDING";
        throw new LifeOSError("PUSH_PENDING", "Pending commit still cannot be pushed.", { commit: pending.commit });
      }
    }
    this.state = "BLOCKED";
    throw new LifeOSError("WORKSPACE_DIVERGED", "Remote history moved away from the pending transaction.", {
  ...
  private async finalizePending(pending: PendingTransaction, remoteOverride?: string): Promise<void> {
    ...
    try {
      ...
      throw new LifeOSError("PUSHED_LOCAL_REPAIR_NEEDED", "GitHub accepted the commit, but the local cache could not fast-forward.", {
  ...
  private async readUtf8(filePath: string, displayPath: string): Promise<string> {
    ...
    try {
      ...
      throw new LifeOSError("VALIDATION_FAILED", "LifeOS content must be valid UTF-8.", { path: displayPath });
    }
  }
  ...
  private async withLock<T>(operation: () => Promise<T>): Promise<T> {
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === "EEXIST") {
        throw new LifeOSError("NOT_READY", "Another workspace operation currently holds the lock.");
      }
      throw error;
    }
